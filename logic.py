...
def create_week_calendar_markup(year: int, month: int) -> InlineKeyboardMarkup:
    """
    Создаёт календарь на месяц с выбором недели.
    """
    today = datetime.now()
    start = datetime(year, month, 1)
    start = start - timedelta(days=start.weekday())  # Первый понедельник

    buttons = []
    weekday_names = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]
    buttons.append([InlineKeyboardButton(name, callback_data="noop") for name in weekday_names])

    for _ in range(6):  # 6 недель
        row = []
        for i in range(7):
            ...
            if date.month != month:
                ...
                if date.date() == today.date():
                    text = f"[{date.day}]"
                callback_data = f"day_{date.year}_{date.month}_{date.day}"
    ...
    # Навигация
    nav_row = [
        InlineKeyboardButton("◀️", callback_data=f"prev_week_{year}_{month}"),
        InlineKeyboardButton(f"{year}-{month:02d}", callback_data="noop"),
        InlineKeyboardButton("▶️", callback_data=f"next_week_{year}_{month}")
    ]
    ...
    if today.month == month:
        quick_row.append(InlineKeyboardButton("Сегодня", callback_data=f"day_{today.year}_{today.month}_{today.day}"))
    tomorrow = today + timedelta(days=1)
    if tomorrow.month == month:
        quick_row.append(InlineKeyboardButton("Завтра", callback_data=f"day_{tomorrow.year}_{tomorrow.month}_{tomorrow.day}"))
    if quick_row:
